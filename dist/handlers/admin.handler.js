...
const awaitingTakeCoins = new Map();
...
async function giveCoins(ctx, targetTelegramId, amount) {
...
💎 Новый баланс: **${updatedUser.coins}** монет`, { parse_mode: "Markdown" });
        ...
        if (targetTelegramId !== ctx.from.id) {
            try {
                await ctx.api.sendMessage(targetTelegramId, `🎁 Вам начислено ${amount} лук койнов!\n\n💎 Ваш баланс: ${updatedUser.coins} монет`);
            }
            catch (error) {
...
async function takeCoins(ctx, targetTelegramId, amount) {
    const targetUser = await findUser(targetTelegramId);
    if (!targetUser) {
        await ctx.reply(`❌ Пользователь с ID ${targetTelegramId} не найден в базе данных.`);
        return;
...
💎 Новый баланс: **${updatedUser.coins}** монет`, { parse_mode: "Markdown" });
        // Уведомляем пользователя
        try {
            await ctx.api.sendMessage(targetTelegramId, `⚠️ С вашего баланса списано ${amount} лук койнов.\n\n💎 Ваш баланс: ${updatedUser.coins} монет`);
        }
        catch (error) {
...
export async function notifyAdminAboutPurchase(bot, userId, amount, price, method) {
    try {
