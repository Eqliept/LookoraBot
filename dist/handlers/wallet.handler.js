...
function getGiftNotification(lang, coins, message) {
    const notifications = {
        RU: {
...
async function getPackageMessage(ctx, pkg) {
...
export const walletHandler = (bot) => {
    // Команда /wallet
    ...
    bot.on("message:text", async (ctx, next) => {
        ...
        if (giftState?.awaitingMessage && giftState.recipientId && giftState.total && giftState.amount) {
            ...
            if (userId === ADMIN_ID) {
                ...
                const notification = getGiftNotification(recipient.language, giftState.total, message !== "-" ? message : undefined);
                try {
            ...
            await ctx.reply(ctx.t("gift-payment-info", {
                total: pkg.total,
                price: pkg.price,
                recipientId: giftState.recipientId
            }), {
    ...
    bot.callbackQuery(/^gift_pay_crypto_(\d+)$/, async (ctx) => {
        ...
        if (invoice) {
            ...
            await ctx.reply(ctx.t("gift-pay-crypto-info", {
                total: pkg.total,
                price: pkg.price,
                recipientId: giftState.recipientId
            }), {
    ...
    bot.callbackQuery(/^check_gift_crypto_(\d+)$/, async (ctx) => {
        ...
        if (status === "paid") {
            ...
            const recipient = await findUser(giftState.recipientId);
            if (recipient) {
                const notification = getGiftNotification(recipient.language, pending.total, giftState.message);
                try {
                    await ctx.api.sendMessage(giftState.recipientId, notification);
                }
                catch (e) {
    ...
    bot.callbackQuery(/^gift_pay_stars_(\d+)$/, async (ctx) => {
        ...
        await ctx.reply(ctx.t("gift-pay-stars-info", {
            total: pkg.total,
            stars: starsPrice,
            recipientId: giftState.recipientId
        }));
    ...
    bot.callbackQuery(/^pay_crypto_(\d+)$/, async (ctx) => {
        ...
        if (invoice) {
            pendingPayments.set(userId, { amount, total: pkg.total, invoiceId: invoice.invoice_id });
            await ctx.reply(ctx.t("pay-crypto-info", { total: pkg.total, price: pkg.price, bonus: pkg.bonus }), {
                reply_markup: new InlineKeyboard()
                    .url(ctx.t("pay-button"), invoice.bot_invoice_url)
                    .row()
    ...
    bot.callbackQuery(/^pay_stars_(\d+)$/, async (ctx) => {
        ...
        await ctx.reply(ctx.t("pay-stars-info", { total: pkg.total, stars: starsPrice, bonus: pkg.bonus }));
        await ctx.answerCallbackQuery();
        // Создаём инвойс для Telegram Stars
        await ctx.api.raw.sendInvoice({
            chat_id: ctx.chat.id,
    ...
    bot.on("message:successful_payment", async (ctx) => {
        ...
        if (payload.isGift && payload.recipientId) {
            ...
            if (recipient) {
                const notification = getGiftNotification(recipient.language, payload.total, payload.giftMessage);
                try {
    ...
    bot.callbackQuery(/^pay_admin_(\d+)$/, async (ctx) => {
        ...
        const amount = parseInt(ctx.match[1]);
        const pkg = getPackageInfo(amount);
        await ctx.reply(ctx.t("admin-topup-confirm", { amount: pkg.amount, total: pkg.total }), {
    ...
    bot.callbackQuery(/^admin_confirm_(\d+)$/, async (ctx) => {
        ...
        if (user) {
            await ctx.reply(ctx.t("topup-success", { total: pkg.total, balance: user.coins }));
        }
...
//# sourceMappingURL=wallet.handler.js.map