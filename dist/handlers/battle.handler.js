...
export const battleHandler = (bot) => {
    bot.callbackQuery("rate_battle", async (ctx) => {
        ...
        if (user.coins < BATTLE_COST) {
            await ctx.reply(ctx.t("not-enough-coins", { balance: user.coins, required: BATTLE_COST }), { reply_markup: getBackKeyboard(ctx) });
            await ctx.answerCallbackQuery();
            return;
        }
        const ui = getBattleUI((user.language || "EN"));
    ...
    bot.on("message:photo", async (ctx, next) => {
        ...
        const validation = await validatePhoto(photoUrl, photoType, (user.language || "EN"));
        ...
        if (session.stage === "player1_front") {
            ...
            try {
                const result = await analyzeBattle(session.player1FrontUrl, session.player1SideUrl, session.player2FrontUrl, photoUrl, (user.language || "EN"));
                ...
                for (const param of params) {
                    const v1 = p1.scores[param.key];
                    const v2 = p2.scores[param.key];
                ...
                await notifyAdminAboutError(error, 'battle analysis', ctx.from.id);
...
//# sourceMappingURL=battle.handler.js.map