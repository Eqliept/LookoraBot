...
const awaitingCustomDescription = new Set();
...
export const styleHandler = (bot) => {
    ...
    bot.callbackQuery("rate_style", async (ctx) => {
        ...
        if (user.coins < STYLE_COST) {
            await ctx.reply(ctx.t("not-enough-coins", { balance: user.coins, required: STYLE_COST }), { reply_markup: getBackKeyboard(ctx) });
            await ctx.answerCallbackQuery();
            return;
        }
        const ui = getStyleUI(user.language || "EN");
    ...
    const categoryCallbacks = ["casual", "work", "date", "social", "event", "custom"];
    for (const category of categoryCallbacks) {
        bot.callbackQuery(`style_cat_${category}`, async (ctx) => {
            ...
            if (!user)
                return;
            const ui = getStyleUI(user.language || "EN");
            const categoryName = getCategoryName(category, user.language || "EN");
            const categoryDesc = getCategoryDescription(category, user.language || "EN");
            if (category === "custom") {
    ...
    bot.on("message:photo", async (ctx, next) => {
        ...
        const validation = await validateStylePhoto(photoUrl, (user.language || "EN"));
        ...
        const result = await analyzeStyle(photoUrl, session.category, session.customDescription, user.language);
        // Сохраняем результат
        ...
        const getCoeffText = (coeff) => {
            if (coeff >= 0.95)
                return `🌟 ${ui.perfectMatch}`;
            if (coeff >= 0.8)
                return `✨ ${ui.goodMatch}`;
            if (coeff >= 0.6)
                return `👍 ${ui.normalMatch}`;
            if (coeff >= 0.45)
                return `😐 ${ui.weakMatch}`;
