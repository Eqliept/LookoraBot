 ...
 */
export const validatePhoto = async (photoUrl, type, lang = "EN") => {
 ...
 */
export const analyzeAppearance = async (frontPhotoUrl, sidePhotoUrl, lang = "EN") => {
    const t = getGPTTranslation(lang);
 ...
 */
export const getImprovementTips = async (result, lang = "EN") => {
    const t = getGPTTranslation(lang);
    // Сортируем параметры по оценке (от худших к лучшим)
    const sortedScores = Object.entries(result.scores)
        .map(([key, value]) => {
        return { key, label: t.labels[key], score: value };
    })
 ...
 */
export const getTelegramFileUrl = async (botToken, fileId) => {
    try {
...
export const styleAnalysisResults = new Map();
/**
 * Валидация фото для анализа стиля (полный рост)
 */
export const validateStylePhoto = async (photoUrl, lang = "EN") => {
    const t = getGPTTranslation(lang);
 ...
 */
export const analyzeStyle = async (photoUrl, category, customDescription, lang = "EN") => {
...
const getDefaultStyleResult = (lang) => {
 ...
 */
export const suggestHairstyle = async (photoUrl, hairAnalysis, lang = "EN") => {
