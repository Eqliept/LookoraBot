...
const HAIR_COST = 75;
const hairAnalysisResults = new Map<string, { result: HairAnalysisResult; photoUrl: string; suggestion?: string }>();
...
export const hairHandler = (bot: Bot<MyContext>) => {
    ...
    bot.on("message:photo", async (ctx) => {
        ...
        try {
            ...
            const analysisResult = await analyzeHair(photoUrl, (user.language || "EN") as Language);
    ...
    bot.callbackQuery("hair_barber", async (ctx) => {
        ...
        try {
            const barberText = await generateBarberInstructions(analysis.suggestion, (user.language || "EN") as Language);
